...
export function GrammarPanel({ explanation, structure, original: _original, loading }: GrammarPanelProps) {
  if (loading) {
    return (
  ...
  if (!explanation && !structure) return null;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      ...
      {explanation && (
        <>
          ...
          >
            <svg style={{ width: '16px', height: '16px', color: '#c94a4a', flexShrink: 0 }} viewBox="0 0 24 24" fill="currentColor">
            ...
            <span
              className="tibetan"
              style={{ fontSize: '0.7rem', color: 'rgba(201,74,74,0.4)', fontFamily: 'var(--font-tibetan)' }}
            >
              ་ཚིག་བརྟག
            </span>
            <span style={{ fontSize: '0.62rem', color: '#4a4540', fontFamily: 'var(--font-sans)', marginLeft: 'auto' }}>
              MiniMax claude-3-5-sonnet
            </span>
          </div>
      ...
      {/* Structure (always shown when available) */}
      {structure && (
